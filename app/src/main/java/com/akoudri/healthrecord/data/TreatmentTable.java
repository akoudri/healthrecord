...
public class TreatmentTable {
    ...
    public static final String TREATMENT_DURATION = "duration";
    ...
    private String[] treatmentCols = {TREATMENT_ID, TREATMENT_PERSON_REF, TREATMENT_ILLNESS_REF, TREATMENT_THERAPIST_REF, TREATMENT_START_DATE,
            TREATMENT_DURATION, TREATMENT_COMMENT};
    ...
    public void createTreatmentTable() {
        ...
        sb.append(TREATMENT_DURATION + " integer,"); //treatment with no end date can be considered permanent
    ...
    public long insertTreatment(int personId, int illnessId, int therapistId, String startDate, int duration, String comment) {
        ...
        if (duration > 0)
            values.put(TREATMENT_DURATION, duration);
        if (comment != null)
            values.put(TREATMENT_COMMENT, comment);
        return db.insert(TREATMENT_TABLE, null, values);
    }

    public boolean updateTreatment(int treatmentId, int illnessId, int therapistId, String startDate, int duration, String comment) {
        ContentValues values = new ContentValues();
        if (illnessId > 0)
            values.put(TREATMENT_ILLNESS_REF, illnessId);
        if (therapistId > 0)
    ...
    public boolean updateTreatment(Treatment treatment)
    {
        ...
        return updateTreatment(treatmentId, illnessId, therapistId, startDate, duration, comment);
    }
    ...
    public List<Treatment> getDayTreatmentsForPerson(int personId, String date)
    {
        ...
        while (! cursor.isAfterLast())
        {
            ...
            sDate = treatment.getStartDate();
            startDate = HealthRecordUtils.stringToCalendar(sDate);
            endDate = HealthRecordUtils.stringToCalendar(HealthRecordUtils.computeEndDate(sDate, treatment.getDuration()));
            if (endDate == null)
            {
    ...
    private Treatment cursorToTreatment(Cursor cursor) {
        ...
        treatment.setDuration((cursor.isNull(5))?-1:cursor.getInt(5));
